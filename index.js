...
app.get("/api/spotifysong/:song", async (req, res) => {
  ...
  axios
    .get("https://open.spotify.com/genre/" + query)
    .then((response) => {
...
app.get("/api/youtubesong/:song", async (req, res) => {
  const query = req.params.song;
  youtube_sr
    .search(query, { limit: 5, safeSearch: true })
    .then((data) => {
      return res.send(data);
    })
    .catch(() => {
...
app.get("/api/youtubedownload", async (req, res) => {
  res.setHeader(
    "Content-disposition",
    "attachment; filename=" + req.query.videoName + ".mp3"
  );
...
app.get("/api/wallpaper", (req, res) => {
  ...
  axios
    .get(
      `https://wallhaven.cc/api/v1/search?q=${q}&atleast=${quality}&page=${page}`
    )
    .then(async (response) => {
